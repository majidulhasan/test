                  ...
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <div className="text-right">
                      <p className={`font-black text-base whitespace-nowrap ${ (item.type === 'INCOME' || (item.isLoan && item.type === 'GIVEN')) ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
                        { (item.type === 'INCOME' || (item.isLoan && item.type === 'GIVEN')) ? '+' : '-'} ৳ {(item.isLoan ? remaining : item.amount).toLocaleString(lang === 'bn' ? 'bn-BD' : 'en-US')}
                      </p>
            ...
            <div key={item.id} className={`bg-white dark:bg-gray-800 p-5 rounded-[2rem] border border-gray-100 dark:border-gray-700 shadow-sm transition-all dark:hover:bg-gray-750 ${isSettled ? 'opacity-50' : ''}`}>
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="text-right flex items-center gap-4 shrink-0">
                  <div className="text-right">
                    <p className={`font-black whitespace-nowrap text-base ${item.type === 'INCOME' || (item.isLoan && item.type === 'GIVEN') ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
                      { (item.type === 'INCOME' || (item.isLoan && item.type === 'GIVEN')) ? '+' : '-'} ৳ {(item.isLoan ? remaining : item.amount).toLocaleString(lang === 'bn' ? 'bn-BD' : 'en-US')}
                    </p>
                    {item.isLoan && remaining !== item.amount && !isSettled && (
                      <p className="text-[9px] text-gray-400 font-bold italic line-through">৳ {item.amount.toLocaleString(lang === 'bn' ? 'bn-BD' : 'en-US')}</p>
...
function NotesView({ t, notes, setNotes, theme }: any) {
  ...
  return (
    <div className="space-y-6"><div className="flex items-center justify-between px-2"><h3 className="font-black text-xl text-gray-900 dark:text-gray-100">{t('monthlyNote')}</h3><input type="month" value={activeMonth} onChange={e => setActiveMonth(e.target.value)} className={`bg-gray-100 dark:bg-gray-800 p-2.5 rounded-2xl border-none font-bold text-xs ${accentText} outline-none transition-colors dark:text-gray-100 dark:color-scheme-dark`} /></div><textarea className="w-full h-80 p-8 rounded-[3rem] bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 focus:ring-4 focus:ring-opacity-10 outline-none text-sm font-medium leading-loose shadow-sm transition-colors text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-600" placeholder={t('save') === 'Save Changes' ? 'Write something important for this month...' : 'এই মাসের জরুরি কিছু লিখে রাখুন...'} value={currentNote?.text || ''} onChange={e => handleSave(e.target.value)} /></div>
  );
}

function SettingsView({ t, settings, onUpdateSettings, onExport, onImport, theme, onShowDevProfile, onManageCategories, onShowUsageGuide }: any) {
  const accentClass = THEME_MAP[theme as ThemeColor].split(' ')[0];
  const accentText = THEME_MAP[theme as ThemeColor].split(' ')[2];
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-[2.5rem] border border-gray-100 dark:border-gray-700 overflow-hidden shadow-sm transition-colors"><button onClick={onManageCategories} className="w-full p-6 flex items-center gap-5 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors border-b dark:border-gray-700 text-left"><div className="bg-emerald-50 dark:bg-emerald-900/30 p-3 rounded-2xl text-emerald-600 dark:text-emerald-400"><ListFilter size={20}/></div><div><p className="font-black text-sm text-gray-900 dark:text-gray-100">{t('manageCategories')}</p><p className="text-[10px] text-gray-500 dark:text-gray-400">{settings.language === 'bn' ? 'আয়-ব্যয় ক্যাটাগরিগুলো পরিচালনা করুন' : 'Manage income & expense categories'}</p></div></button><button onClick={onShowUsageGuide} className="w-full p-6 flex items-center gap-5 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors border-b dark:border-gray-700 text-left"><div className="bg-amber-50 dark:bg-amber-900/30 p-3 rounded-2xl text-amber-600 dark:text-amber-400"><BookOpen size={20}/></div><div><p className="font-black text-sm text-gray-900 dark:text-gray-100">{t('usageGuide')}</p><p className="text-[10px] text-gray-500 dark:text-gray-400">{settings.language === 'bn' ? 'অ্যাপটি কিভাবে ব্যবহার করবেন জানুন' : 'Learn how to use the app'}</p></div></button><button onClick={onExport} className="w-full p-6 flex items-center gap-5 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors border-b dark:border-gray-700 text-left"><div className="bg-blue-50 dark:bg-blue-900/30 p-3 rounded-2xl text-blue-600 dark:text-blue-400"><Download size={20}/></div><div><p className="font-black text-sm text-gray-900 dark:text-gray-100">{t('backup')}</p><p className="text-[10px] text-gray-500 dark:text-gray-400">{settings.language === 'bn' ? 'তথ্যগুলো JSON ফাইলে সেভ করুন' : 'Save data to JSON file'}</p></div></button><label className="w-full p-6 flex items-center gap-5 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer text-left border-b dark:border-gray-700"><div className={`${accentClass.replace('bg-', 'bg-opacity-10 bg-')} p-3 rounded-2xl`}><Upload size={20}/></div><div><p className="font-black text-sm text-gray-900 dark:text-gray-100">{t('restore')}</p><p className="text-[10px] text-gray-500 dark:text-gray-400">{settings.language === 'bn' ? 'পুরানো ডাটা ফিরিয়ে আনুন' : 'Bring back old data'}</p></div><input type="file" accept=".json" onChange={onImport} className="hidden" /></label><button onClick={onShowDevProfile} className="w-full p-6 flex items-center gap-5 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors text-left"><div className="bg-violet-50 dark:bg-violet-900/30 p-3 rounded-2xl text-violet-600 dark:text-violet-400"><User size={20}/></div><div><p className="font-black text-sm text-gray-900 dark:text-gray-100">{t('devProfile')}</p><p className="text-[10px] text-gray-500 dark:text-gray-400">{settings.language === 'bn' ? 'আমার সম্পর্কে জানুন' : 'Know about me'}</p></div></button></div>
      <div className="text-center p-10 opacity-60"><FileText size={40} className="mx-auto mb-4 text-gray-300 dark:text-gray-600" /><p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 dark:text-gray-500">{t('appTitle')} v2.5 • {settings.language === 'bn' ? 'পার্সোনাল এডিশন' : 'Personal Edition'}</p></div>
    </div>
  );
}
